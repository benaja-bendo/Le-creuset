...
/**
 * Bouton générique pour actions dans l’espace client
 */
...
// Titre de section
/**
 * Titre de section standard avec sous-titre
 */
...
// Visionneuse 3D
/**
 * Visionneuse 3D simplifiée pour prévisualiser le rendu matière/finition
 */
const ThreeViewer = ({ materialType, finishType }: { materialType: string, finishType: string }) => {
  ...
  useEffect(() => {
    ...
    const pointLight2 = new THREE.PointLight(0xffaf7b, 0.8);
  ...
  useEffect(() => {
    ...
    switch(materialType) {
      case 'or-jaune':
        ...
        break;
      case 'or-rose':
        material.color.setHex(0xE0BFB8);
        material.metalness = 1.0;
        material.roughness = finishType === 'poli' ? 0.1 : 0.4;
        break;
      case 'argent':
  ...
  return <div ref={containerRef} className="w-full h-full bg-black rounded-sm shadow-inner" />;
};

// Page Devis (client)
/**
 * Page de devis STL côté client (upload, configuration, estimation)
 */
export default function Quote() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-secondary-200 rounded-sm overflow-hidden flex flex-col md:flex-row min-h-[560px]">
        <div className="w-full md:w-2/3 bg-black relative flex items-center justify-center border-b md:border-b-0 md:border-r border-secondary-200">
          {step === 1 ? (
            <div 
              className="text-center p-10 border-2 border-dashed border-secondary-300 hover:border-primary-600 rounded-lg transition-colors cursor-pointer group w-full max-w-md mx-auto"
              onClick={handleUpload}
            >
              {isAnalyzing ? (
                <div className="flex flex-col items-center animate-pulse">
                  <RefreshCw className="animate-spin text-primary-600 mb-4" size={48} />
                  <p className="text-secondary-600">Analyse de la géométrie...</p>
                  <p className="text-secondary-500 text-xs mt-2">Calcul du volume et des surfaces</p>
                </div>
              ) : (
                <>
                  <div className="w-20 h-20 bg-secondary-800 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform">
                    <Upload className="text-secondary-300 group-hover:text-primary-500" size={32} />
                  </div>
                  <h3 className="text-xl text-white font-medium mb-2">Déposez votre fichier 3D</h3>
                  <p className="text-secondary-300 mb-6">Format .STL ou .OBJ supporté</p>
            ...
            <div className="w-full h-full relative">
              ...
              <button 
                onClick={() => setStep(1)} 
                className="absolute top-4 right-4 text-secondary-300 hover:text-white bg-secondary-900/50 p-2 rounded"
              >
                <X size={20} />
              </button>
            </div>
        ...
        <div className="w-full md:w-1/3 p-8 bg-white flex flex-col">
          <div className="mb-8">
            <h3 className="text-primary-600 text-xs font-bold tracking-widest uppercase mb-4">Configuration</h3>
            <div className="mb-6">
              <label className="block text-sm text-secondary-700 mb-3">Matériau</label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-3 rounded border text-left transition-all flex items-center gap-3 ${
                      material === m.id 
                      ? 'border-primary-600 bg-secondary-50 text-secondary-900 ring-1 ring-primary-500/50' 
                      : 'border-secondary-300 text-secondary-600 hover:border-secondary-400 bg-white'
            ...
            <div className="mb-6">
              <label className="block text-sm text-secondary-700 mb-3">Finition</label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded border text-xs font-medium transition-all ${
                      finish === f.id 
                      ? 'border-primary-600 bg-secondary-50 text-secondary-900' 
                      : 'border-secondary-300 text-secondary-600 hover:border-secondary-400 bg-white'
                    } ${step === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={step === 1}
                  >
          ...
          <div className="mt-auto border-t border-secondary-200 pt-6">
            <div className="flex justify-between items-end mb-6">
              <div className="text-secondary-600 text-sm">Estimation HT</div>
              <div className="text-3xl font-serif text-secondary-900 flex items-center">
                {step === 1 ? (
